...
def construct_basb_prompt(question: str, context_embeddings: dict, df: pd.DataFrame) -> tuple[str, str]:
    """
    Fetch relevant
    """
    most_relevant_document_sections = order_document_sections_by_query_similarity(question, context_embeddings)
...
def ask_basb(request):
    ...
    s3.download_file('askbook', 'basb.pdf.pages.csv', 'pages.csv')
    s3.download_file('askbook', 'basb.pdf.embeddings.csv', 'embeddings.csv')
